...
1. 在“设置中心 -> API 与模型”配置支持图片编辑的模型、API Key、baseUrl 和真实请求模型名。
2. 进入“修图工作台”，上传 PNG、JPEG 或 WebP 源图创建会话。
3. 按任务选择整图编辑、局部编辑或版本合并；局部编辑时先使用画笔、矩形、套索等工具创建选区。
4. 输入本轮修图要求。新手可先点击“AI 润色”补全保护项、边缘融合和输出约束，再提交生成。
5. 每轮可生成 1-4 个候选。点击候选缩略图打开大图预览，检查细节后下载或“检出并继续”。
6. 在已检出的版本上继续对话修图；需要保留不同方向时，可从任意历史版本创建分支或执行局部合并。

常用交互：

- 候选大图预览支持 50%-200% 缩放，步进为 25%；点击百分比可恢复 100%。
...
- 用户检出候选后才更新当前版本。
- 候选缩略图支持点击放大预览，并提供明确的放大图标、键盘可访问名称和焦点状态。
- 大图预览支持 50%-200% 缩放、100% 重置、下载、检出并继续、`Esc` 关闭和点击遮罩关闭。
- 任意历史版本可检出或创建新分支。
...
- 工具使用图标按钮并提供 tooltip。
- 修图模式、画布缩放、选区工具、蒙版组合、保护项、AI 润色、生成、取消、候选检出/重试、版本操作和质量检查均提供面向新手的结果型提示。
- 候选图片本身是放大预览入口；悬停和键盘聚焦时展示视觉反馈，弹窗内保留下载与检出主操作。
