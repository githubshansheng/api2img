...
| 项目 | 状态 |
| --- | --- |
| 当前里程碑 | M1-M5 已完成；一致性套图功能分支待审核 |
| 当前任务 | DEV-SUITE-001 一致性套图生成 |
| 更新时间 | 2026-07-12 |
| 总体状态 | 已完成脚手架、领域类型、模型配置、真实上游生成链路、设置与模型映射、端点切换、结果工作台、错误映射、历史、批量生成、模板库、zip 下载、素材模板、模型对比、真实图片识别与推理测试、高级存储配置、运营检查入口和 `GPT-Image2-Studio` 迁移；新增一致性套图工作台，提供“通用同主体 4 张”和“电商产品 5 张”预设，以主视觉锚点锁定主体和风格，再生成其余场景；支持共享视觉规范、参考图角色、可编辑场景、候选图、人工/自动锚点选择、全局与单套并发控制、SSE 实时进度、取消、失败重试、删除和单图下载；套图状态持久化到 SQLite，参考图与结果可归档到本地素材目录，进程异常退出会将运行中任务标记为 `interrupted`；API Key、认证 Header 和敏感 URL 参数不会写入数据库，远程图片归档包含公网地址、重定向、DNS 固定、大小和图片文件签名校验 |
