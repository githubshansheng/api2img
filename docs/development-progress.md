...
| --- | --- |
| 当前里程碑 | M1-M5 已完成收口验证 |
| 当前任务 | DOCS-README-001 开源化 README 补充与提交 |
| 更新时间 | 2026-07-11 |
| 总体状态 | 已完成脚手架、领域类型、模型配置、真实上游生成链路、设置与模型映射、端点切换、结果工作台、错误映射、历史、生成页多线程批量能力、模板库、zip 下载、素材模板、模型对比、图片识别真实 `POST /v1/chat/completions` 链路与推理测试真实多端点链路、高级存储配置和运营检查入口；已将 `GPT-Image2-Studio` 原生运行时、静态素材、服务端工作流和部署脚本迁入 `vendor/gpt-image2-studio`，新增 `GPT Studio` 菜单与全尺寸 iframe 承载页，vendored 生产路径无假数据关键词；独立“批量生成”菜单已合并进“生成图片”页，高级存储默认本地归档，设置保存后立即关闭，图片预览滚轮缩放已隔离外层滚动并支持鼠标拖拽平移，生成页已改为按屏幕高度自适应并优化生产参数组排版，默认模型冗余提示已移除，默认提示词每次进入页面从模板库随机抽取，费用预估区域已压缩；OpenAI 图片调用已将“生成张数”映射为单次 API 的 `n`，将“批量次数”作为前端排队请求次数，并在请求体/cURL/UI 中补齐输出格式、背景、审核和压缩等原生参数；图片请求超时已统一固定为 30 分钟，图生图参考图上传即转为非空 base64 data URL，prompt 参数同步已排除生成数量、审核强度和响应格式，预览缩放上限提升至 500%；识图工具模型默认使用 `gpt-5.2`，推理工具模型默认使用 `claude-opus-4-8`，均独立于生图模型配置；识图/推理结果区已补齐运行中取消、复制结果/请求体/原始摘要、推理专用参考图上传与结构化错误态；M1-M5 规划任务已完成收口验证 |
