...
test("top navigation groups functions into an Apple-style global mega menu", async () => {
  ...
  assert.match(styles, /--topbar-fixed-clearance:\s*44px;/);
  assert.match(styles, /html:not\(\[data-ui-layout="tablet"\]\):not\(\[data-ui-layout="mobile"\]\) \.topbar\s*\{[\s\S]*position:\s*fixed;[\s\S]*top:\s*0;[\s\S]*left:\s*50%;[\s\S]*background:\s*var\(--topbar-bg,\s*rgba\(8,\s*13,\s*26,\s*0\.86\)\);[\s\S]*opacity:\s*1;[\s\S]*transform:\s*translate\(-50%,\s*0\);/);
  assert.match(styles, /html:not\(\[data-ui-layout="tablet"\]\):not\(\[data-ui-layout="mobile"\]\) \.view-root\s*\{[\s\S]*margin-top:\s*var\(--topbar-fixed-clearance,\s*44px\);/);
  assert.match(styles, /html:not\(\[data-ui-layout="stacked"\]\):not\(\[data-ui-layout="tablet"\]\):not\(\[data-ui-layout="mobile"\]\) \.view-root\s*\{[\s\S]*height:\s*calc\(100svh - var\(--view-root-offset,\s*12px\) - var\(--topbar-fixed-clearance,\s*44px\)\);/);
  assert.doesNotMatch(styles, /topbar-trigger-height|topbar-reveal|calc\(-100% \+ var\(--topbar/);
  ...
  assert.doesNotMatch(app, /TOPBAR_REVEAL|syncTopbarReveal|setTopbarReveal|bindTopbarReveal|topbar-reveal/);
