...
function makeDraftForQuantity(expectedQuantity, overrides = {}) {
  const { productPhrase = "Electronic Fishing Lure Bass Trout Freshwater Swimbait", ...draftOverrides } = overrides;
  return makeValidDraft({
    title: `${expectedQuantity} ${productPhrase}`,
    fiveBullets: [
      `CORE VALUE: ${expectedQuantity} option keeps the listing quantity clear for shoppers.`,
      "BUILT TO LAST: Product details stay grounded in visible source evidence.",
      "REAL-LIFE USE: Clear use-focused copy helps shoppers picture where the item fits.",
      "SIZE & FIT: Compact details make quantity and variant comparison easier.",
      "PACKAGE SNAPSHOT: Concise wording keeps searchable product terms natural.",
    ],
    ...draftOverrides,
  });
...
function makeListingFetch(drafts, calls = []) {
  const draftList = Array.isArray(drafts) ? drafts : [drafts];
  return async (_url, init) => {
    const body = JSON.parse(init.body);
    calls.push(body);
    const draft = draftList[Math.min(calls.length - 1, draftList.length - 1)];
...
test("generateCreationListingDrafts creates one parent draft for all SKU variants", async () => {
  const drafts = await generateCreationListingDrafts({
    ...
    fetchImpl: makeListingFetch(makeDraftForQuantity("2 Pack", {
      productPhrase: "Fishing Lure Bass Trout Freshwater Swimbait",
    })),
  });
  ...
  assert.doesNotMatch(drafts[0].title, /Listing Draft/i);
  assert.equal(validateListingAgentDraft(drafts[0], "2 Pack").ok, true);
});

test("generateCreationListingDrafts keeps metric and imperial specs out of returned titles", async () => {
  const drafts = await generateCreationListingDrafts({
    set: {
    ...
    config: { baseUrl: "https://example.test/v1", apiKey: "test-key", responsesModel: "gpt-5.4" },
    fetchImpl: makeListingFetch(makeDraftForQuantity("1 Pack", {
      productPhrase: "Electric Fishing Lure Bass Trout Freshwater Swimbait",
    })),
  });
...
test("generateCreationListingDrafts uses grouped SKU subject unit count and search terms in titles", async () => {
  const drafts = await generateCreationListingDrafts({
    ...
    config: { baseUrl: "https://example.test/v1", apiKey: "test-key", responsesModel: "gpt-5.4" },
    fetchImpl: makeListingFetch(makeDraftForQuantity("3 Pack", {
      title: "3 Pack Electronic Fishing Lure Propeller Swimbait Slow Sinking Bass Trout Freshwater Bait",
      backendSearchTerms: "electronic fishing lure propeller swimbait slow sinking bass trout freshwater saltwater hard bait",
      keywordBuckets: {
        exact: ["Electronic Fishing Lure"],
        longTail: [
        ...
        descriptive: ["propeller lure", "slow sinking bait"],
      },
    })),
  });
...
test("generateCreationListingDrafts honors set-level same-SKU pack counts for grouped subjects", async () => {
  const drafts = await generateCreationListingDrafts({
    ...
    config: { baseUrl: "https://example.test/v1", apiKey: "test-key", responsesModel: "gpt-5.4" },
    fetchImpl: makeListingFetch(makeDraftForQuantity("6 Pack")),
  });
...
test("generateCreationListingDrafts uses visible subject unit count for swimbait titles", async () => {
  const drafts = await generateCreationListingDrafts({
    ...
    config: { baseUrl: "https://example.test/v1", apiKey: "test-key", responsesModel: "gpt-5.4" },
    fetchImpl: makeListingFetch(makeDraftForQuantity("4 Pack", {
      productPhrase: "Swimbait Lure Bass Freshwater Slow Sinking Propeller Jointed Lifelike Saltwater Hard Bait",
    })),
  });
...
test("generateCreationListingDrafts writes mixed grouped SKU quantities as slash pack titles", async () => {
  const drafts = await generateCreationListingDrafts({
    ...
    config: { baseUrl: "https://example.test/v1", apiKey: "test-key", responsesModel: "gpt-5.4" },
    fetchImpl: makeListingFetch(makeDraftForQuantity("2 Pack / 3 Pack")),
  });
...
test("listing drafts describe grouped two-unit subjects", () => {
  const draft = makeDraftForQuantity("2 Pack", {
    productPhrase: "Fishing Lure Bass Trout Freshwater Swimbait",
    description: "Two complete visible lure bodies are described as one grouped sellable option.",
  });
...
test("listing drafts avoid unsupported claims and competitor brand terms", () => {
  const draft = makeDraftForQuantity("2 Pack", {
    productPhrase: "Fishing Lure Bass Trout Freshwater Swimbait",
  });
...
test("listing validation accepts English marketplace titles for Chinese source inputs", async () => {
  const source = {
    setId: "set-cn",
    productName: "路亚硬饵",
  ...
  const mockDraft = makeDraftForQuantity("1 Pack", {
    productPhrase: "Electric Fishing Lure Bass Trout Freshwater Swimbait",
  });
...
test("listing draft does not infer product keywords from numbered first aid kit contents", () => {
  const source = {
    ...standardSource,
    productName: "急救包",
    skuBundleCount: 1,
  ...
  const draft = makeDraftForQuantity("1 Pack", {
    productPhrase: "First Aid Kit Home Travel Emergency Compact Portable Quick Access",
  });
...
test("listing draft uses product-facing copy instead of internal template commentary", () => {
  const source = {
    ...standardSource,
    productName: "急救包",
    skuBundleCount: 1,
  ...
  const draft = makeDraftForQuantity("1 Pack", {
    productPhrase: "First Aid Kit Home Travel Emergency Compact Portable Quick Access",
    description: "Compact first aid kit wording focuses on home, travel, emergency, portable, and quick access use.",
  });
...
test("listing draft does not classify single bandage items as a first aid kit", () => {
  const source = {
    ...standardSource,
    productName: "创口贴",
    skuTitle: "",
  ...
  const draft = makeDraftForQuantity("1 Pack", {
    productPhrase: "Bandages Home Travel Compact Pack",
  });
...
test("listing drafts keep long SKU fields under 500 characters", () => {
  ...
  const mockDraft = makeDraftForQuantity("2 Pack", {
    productPhrase: "Desk Organizer Storage Shelf",
    description: "Compact desk organizer copy keeps storage details concise.",
  });
