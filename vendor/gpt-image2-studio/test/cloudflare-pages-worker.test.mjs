...
test("Cloudflare creation listing route accepts explicit set metadata and returns input-only drafts", async () => {
  ...
  const request = new Request("https://studio.example/api/creation/listings", {
    ...
    body: JSON.stringify({
      baseUrl: "https://listing.example/v1",
      apiKey: "listing-key",
      responsesModel: "gpt-listing",
      set: {
        setId: "worker-set",
  ...
  assert.equal(seenRequests[0].url, "https://listing.example/v1/responses");
  assert.equal(seenRequests[0].body.model, "gpt-listing");
  ...
  const response = await handleApiRequest(request, {
    async fetchImpl(url, init) {
      ...
      return new Response(JSON.stringify({
        output_text: JSON.stringify(makeListingDraft({
          title: "2 Pack / 3 Pack Electronic Fishing Lure Bass Trout Freshwater Swimbait",
          fiveBullets: [
            "CORE VALUE: 2 Pack / 3 Pack option keeps the listing quantity clear for shoppers.",
          ...
          backendSearchTerms: "electronic fishing lure bass trout freshwater swimbait",
          keywordBuckets: {
            exact: ["electronic fishing lure"],
            longTail: ["bass trout fishing lure"],
            traffic: ["freshwater swimbait"],
            descriptive: ["compact fishing bait"],
          },
          description:
            "Electronic fishing lure option for grouped SKU shoppers; the first choice shows 2 complete visible lure units and the second choice shows 3 complete visible lure units.",
        })),
      }), {
        status: 200,
        headers: { "content-type": "application/json" },
