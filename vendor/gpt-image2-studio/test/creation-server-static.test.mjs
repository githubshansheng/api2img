...
test("local server keeps creation generation on the real request workflow", async () => {
  const server = await readFile(serverPath, "utf8");
  ...
  assert.match(server, /return requestImageGeneration\(options\)/);
  assert.doesNotMatch(server, /IMAGE_STUDIO_MOCK_IMAGE_GENERATION/);
  assert.doesNotMatch(server, /MOCK_IMAGE_BASE64/);
  assert.match(server, /event\.type === "final_image"/);
});
