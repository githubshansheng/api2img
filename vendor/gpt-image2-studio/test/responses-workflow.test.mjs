...
function assertPromptHasNoTechnicalSettings(prompt) {
  for (const forbidden of [
    "Technical image request settings",
    "size/resolution",
    "aspect ratio",
    "output format",
    "response_format",
  ]) {
    assert.equal(String(prompt).includes(forbidden), false);
  }
}

test("image request settings stay in native request fields instead of prompt text", () => {
  ...
  assert.equal(requestBody.input[0].content[0].text, "Create image");
  assertPromptHasNoTechnicalSettings(requestBody.input[0].content[0].text);
  assert.equal(requestBody.tools[0].size, "1024x1536");
...
test("createResponsesRequestBody keeps gpt-5.4 on the outer model and passes reasoning effort", () => {
  const prompt = "Create a response image";
  const requestBody = createResponsesRequestBody({
    prompt,
    size: "1024x1536",
    quality: "high",
  ...
  assert.equal(requestBody.input[0].content[0].text, prompt);
  assertPromptHasNoTechnicalSettings(requestBody.input[0].content[0].text);
  assert.equal(requestBody.tools[0].size, "1024x1536");
  assert.equal(requestBody.tools[0].quality, "high");
  assert.equal(requestBody.tools[0].output_format, "jpeg");
});
...
test("createDirectImageRequestBody targets the image model without Responses routing fields", () => {
  ...
  assertPromptHasNoTechnicalSettings(requestBody.prompt);
...
test("direct image generation keeps OpenAI Image API request body even for Gemini model names", async () => {
  ...
  assertPromptHasNoTechnicalSettings(requests[0].body.prompt);
  assert.equal(requests[0].body.response_format, "b64_json");
...
test("direct image generation submits reference image requests to image edits", async () => {
  ...
  assertPromptHasNoTechnicalSettings(requests[0].body.get("prompt"));
...
test("chat completions image request body omits image generations response_format", () => {
  ...
  assert.deepEqual(requestBody.messages, [
    {
      role: "user",
      content: "Create a chat image",
    },
  ]);
  assertPromptHasNoTechnicalSettings(requestBody.messages[0].content);
  assert.equal(requestBody.size, "1024x1024");
  assert.equal(requestBody.quality, "high");
  assert.equal(requestBody.output_format, "png");
  ...
  assert.deepEqual(requestBody.messages, [
    {
      ...
      content: [
        {
          type: "text",
          text: "Use the references to create a unified product image.",
        },
...
test("route A chat completions image generation forwards reference images", async () => {
  ...
  assert.deepEqual(requests[0].body.messages[0].content, [
    {
      type: "text",
      text: "Create a campaign image from these references.",
    },
